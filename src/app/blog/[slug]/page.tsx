...
import { getPost, getPosts, getSite } from "@/sanity/queries";
...
/** Estimate reading time from the body when the editor hasn't set one. */
function estimateReadTime(body: PortableTextBlock[]): number {
  const words = body
    .flatMap((block) =>
      block._type === "block" && Array.isArray((block as { children?: { text?: string }[] }).children)
